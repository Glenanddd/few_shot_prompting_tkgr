...
def unknown_check_prompt_length(prompt, condicate_list, return_rules, model, rng=None):
...
def iteration_check_prompt_length(prompt, condicate_list, return_rules, model, rng=None):
    '''Check whether the input prompt is too long. If it is too long, remove the first path and check again.'''
    if rng is None:
        rng = random
    all_condicate = ";".join(condicate_list)
...
def check_prompt_length(prompt, list_of_paths, model, rng=None):
    '''Check whether the input prompt is too long. If it is too long, remove the first path and check again.'''
    if rng is None:
        rng = random
    all_paths = "".join(list_of_paths)
    ...
    if model.token_len(all_tokens) < maximun_token:
        ...
        # Shuffle the paths
        rng.shuffle(list_of_paths)
        new_list_of_paths = []
        # check the length of the prompt
        for p in list_of_paths:
            tmp_all_paths = "".join(new_list_of_paths + [p])
            tmp_all_tokens = prompt + tmp_all_paths
            if model.token_len(tmp_all_tokens) > maximun_token:
                return "".join(new_list_of_paths)
...
def get_token_limit(model='gpt-4'):
    ...
    elif model in ['gpt-3.5-turbo-16k', 'gpt-3.5-turbo-16k-0613', 'gpt-3.5-turbo']:
...
def shuffle_split_path_list(path_content_list, prompt_len, model, rng=None):
    """
...
def load_json_data(file_path, default=None):
    ...
    try:
        checked_path = maybe_windows_long_path(file_path)
        if os.path.exists(checked_path):
            # print(f"Use cache from: {file_path}")
            with safe_open(checked_path, 'r') as file:
...
def write_to_file(content, path):
    with safe_open(path, "w", encoding="utf-8") as fout:
        fout.write(content)
...
def _bfs_relational(adj, roots, max_nodes_per_hop=None, rng=None):
    ...
    while current_lvl:
        ...
        if max_nodes_per_hop and max_nodes_per_hop < len(next_lvl):
            next_lvl = set(rng.sample(sorted(next_lvl), max_nodes_per_hop))
...
def copy_folder_contents(source_folder, destination_folder):
    # 创建目标文件夹，如果它不存在
    destination_folder_checked = maybe_windows_long_path(destination_folder)
    if not os.path.exists(destination_folder_checked):
        os.makedirs(destination_folder_checked, exist_ok=True)

    # 复制每个文件和子文件夹
    source_folder_checked = maybe_windows_long_path(source_folder)
    for item in sorted(os.listdir(source_folder_checked), key=str.lower):
        source_item = maybe_windows_long_path(os.path.join(source_folder_checked, item))
        destination_item = maybe_windows_long_path(os.path.join(destination_folder_checked, item))
...
def get_candicates_within_interval(timestamp_id, interval, bkg, return_recent=False):
    ...
    if return_recent is False:
       return sorted(set(head_id).union(set(target_id)))
...
def get_candicates_auto(timestamp_id, interval, bkg, return_recent=False):
    ...
    if return_recent is False:
       return sorted(set(target_id))

def clear_folder(folder_path):
    # Added for subset prompt-eval (2026-01-20)
    # IMPORTANT: Iteration_reasoning.py expects output directories to exist after "clearing".
    # Previous implementation removed the directory itself, which can cause FileNotFoundError
    # when later writing files under that folder.
    folder_path_checked = maybe_windows_long_path(folder_path)
    if not os.path.exists(folder_path_checked):
        return
    if not os.path.isdir(folder_path_checked):
        return

    for name in sorted(os.listdir(folder_path_checked), key=str.lower):
        child = maybe_windows_long_path(os.path.join(folder_path_checked, name))
