...
def get_candidates(
        ...
        evaluation_type, timestamp_dict, trace_scores_by_cand=None, trace_rule_id=None
):
    """
    ...
    for cand in cands:
        ...
        for s in dicts_idx:
            ...
            # Optional tracing: record which rule contributes which raw score to which candidate.
            # The caller is responsible for aggregating these raw scores into deltas.
            if trace_scores_by_cand is not None and trace_rule_id is not None:
                cand_i = int(cand)
                rid = str(trace_rule_id)
                try:
                    trace_scores_by_cand[cand_i][rid].append(float(score))
                except KeyError:
                    trace_scores_by_cand.setdefault(cand_i, {}).setdefault(rid, []).append(float(score))
...
def save_candidates(
    ...
    if candidates_file_name:
        filename = os.path.basename(str(candidates_file_name))
    else:
        filename = "{0}_cands_r{1}_w{2}_{3}.json".format(
            rules_confidence_file[:-11], rule_lengths, window, score_func_str
        )
        filename = filename.replace(" ", "")
    with open(maybe_windows_long_path(os.path.join(ranked_rules_path, filename)), "w", encoding="utf-8") as fout:
    ...
    if timestamp_file_name:
        filename = os.path.basename(str(timestamp_file_name))
    else:
    ...
    return candidates_filename, timestamp_filename
