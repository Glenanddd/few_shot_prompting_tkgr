...
class Dataset(object):
    def __init__(self, data_root, sparsity=1, inv=False):
        ...
        entity_path = maybe_windows_long_path(os.path.join(data_root, 'entities.txt'))
        self.idx2ent_, self.ent2idx_ = load_entities(entity_path)
        # Construct rdict which contains relation2idx & idx2relation2
        relation_path = maybe_windows_long_path(os.path.join(data_root, 'relations.txt'))
        ...
        # load (h, r, t) tuples
        fact_path = maybe_windows_long_path(os.path.join(data_root, 'facts.txt'))
        train_path = maybe_windows_long_path(os.path.join(data_root, 'train.txt'))
        valid_path = maybe_windows_long_path(os.path.join(data_root, 'valid.txt'))
        test_path = maybe_windows_long_path(os.path.join(data_root, 'test.txt'))
        if inv:
            fact_path += '.inv'
